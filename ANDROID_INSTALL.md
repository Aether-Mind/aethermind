# AetherMind on Android (Termux)

A complete guide to running AetherMind on Android using Termux + proot Ubuntu.
...
AetherMind requires [Bun](https://bun.sh) to build, and Bun does not support Android natively. The workaround is running a real Ubuntu environment inside Termux via `proot-distro`, where Bun's Linux binary works correctly.
...
### Step 6 — Build AetherMind

```bash
cd /data/data/com.termux/files/home/aethermind
bun run build
```

You should see:
```
✓ Built aethermind v0.1.6 → dist/cli.mjs
```
...
Both were tested and fail due to AetherMind's large system prompt (~50K tokens):
...
- Run `bun run build` again only if you pull updates to the AetherMind repo.
