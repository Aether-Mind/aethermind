...
beforeEach(() => {
  tempDir = mkdtempSync(join(tmpdir(), 'aethermind-env-test-'))
...
test('getGlobalClaudeFile: new install returns .aethermind.json when neither file exists', async () => {
  const { getGlobalClaudeFile } = await importFreshEnvModule()
  expect(getGlobalClaudeFile()).toBe(join(tempDir, '.aethermind.json'))
})
...
test('getGlobalClaudeFile: migrated user uses .aethermind.json when both files exist', async () => {
  writeFileSync(join(tempDir, '.claude.json'), '{}')
  writeFileSync(join(tempDir, '.aethermind.json'), '{}')
  const { getGlobalClaudeFile } = await importFreshEnvModule()
  expect(getGlobalClaudeFile()).toBe(join(tempDir, '.aethermind.json'))
})
