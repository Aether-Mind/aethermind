...
export const PROFILE_FILE_NAME = '.aethermind-profile.json'
...
export async function buildStartupEnvFromProfile(options?: {
  ...
  // The legacy single-profile file (~/.aethermind-profile.json) is a
  ...
  return buildLaunchEnv({
    ...
    goal:
      options?.goal ??
      normalizeRecommendationGoal(processEnv.aethermind_PROFILE_GOAL),
    processEnv,
...
export async function applySavedProfileToCurrentSession(options: {
  ...
  const nextEnv = await buildLaunchEnv({
    ...
    goal: normalizeRecommendationGoal(processEnv.aethermind_PROFILE_GOAL),
    processEnv: baseEnv,
    getOllamaChatBaseUrl,
