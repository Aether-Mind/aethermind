...
export function isClaudeSettingsPath(filePath: string): boolean {
  ...
  if (
    normalizedPath.endsWith(`${sep}.aethermind${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.aethermind${sep}settings.local.json`) ||
...
function isClaudeConfigFilePath(filePath: string): boolean {
  ...
  const openCommandsDir = join(getOriginalCwd(), '.aethermind', 'commands')
  const openAgentsDir = join(getOriginalCwd(), '.aethermind', 'agents')
  const openSkillsDir = join(getOriginalCwd(), '.aethermind', 'skills')
