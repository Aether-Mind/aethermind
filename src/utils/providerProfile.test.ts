...
const missingCodexAuthPath = join(tmpdir(), 'aethermind-missing-codex-auth.json')
...
test('codex launch prefers auth account id over stale persisted value', async () => {
  const codexHome = mkdtempSync(join(tmpdir(), 'aethermind-codex-'))
  try {
    writeFileSync(
...
test('saveProfileFile writes a profile that loadProfileFile can read back', () => {
  const cwd = mkdtempSync(join(tmpdir(), 'aethermind-profile-file-'))
...
test('clearPersistedCodexOAuthProfile removes only persisted Codex OAuth profiles', async () => {
  const cwd = mkdtempSync(join(tmpdir(), 'aethermind-codex-oauth-profile-'))
...
test('buildStartupEnvFromProfile preserves plural-profile env when the legacy file is stale', async () => {
  ...
  // addProviderProfile does NOT sync the legacy .aethermind-profile.json,
