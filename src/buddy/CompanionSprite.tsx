...
const HEART_FRAME_WIDTH = 14; // "   ♡    ♡   " is 14 chars
function spriteColWidth(nameWidth: number, hasHeartFrame = false): number {
  const baseWidth = Math.max(SPRITE_BODY_WIDTH, nameWidth + NAME_ROW_PAD);
  if (hasHeartFrame) {
    return Math.max(baseWidth, HEART_FRAME_WIDTH);
  }
  return baseWidth;
}
...
export function CompanionSprite(): React.ReactNode {
  ...
  const colWidth = spriteColWidth(stringWidth(companion.name), petting);
