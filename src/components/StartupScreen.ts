/**
 * aethermind startup screen — filled-block text logo with sunset gradient.
 * Called once at CLI startup before the Ink UI renders.
 *
 * Addresses: https://github.com/Gitlawb/aethermind/issues/55
 */
...
// Bright cyan gradient - high contrast vibrancy
const AETHER_GRAD: RGB[] = [
  [0, 255, 255],    // Bright cyan (top)
  [0, 200, 240],    // Sky cyan
  [0, 150, 220],    // Deep cyan
  [0, 100, 200],    // Ocean cyan (bottom)
]

const ACCENT: RGB = [0, 255, 255]        // Bright cyan
const CREAM: RGB = [200, 255, 255]       // Light cyan-white
const DIMCOL: RGB = [80, 180, 200]       // Muted cyan
const BORDER: RGB = [0, 180, 200]        // Medium cyan
...
export function printStartupScreen(): void {
  ...
  // Gradient logo
  const allLogo = [...LOGO_AETHER, '', ...LOGO_MIND]
  ...
  for (let i = 0; i < total; i++) {
    ...
    if (allLogo[i] === '') {
      ...
      out.push(paintLine(allLogo[i], AETHER_GRAD, t))
    }
  }
  ...
  out.push(`  ${rgb(...ACCENT)}\u2606${RESET} ${rgb(...CREAM)}Neural sync complete. Zero latency.${RESET} ${rgb(...ACCENT)}\u2606${RESET}`)
  out.push('')
  ...
  out.push(`${rgb(...BORDER)}\u255a${'\u2550'.repeat(W - 2)}\u255d${RESET}`)
  out.push(`  ${DIM}${rgb(...DIMCOL)}Aether Mind ${RESET}${rgb(...ACCENT)}v${MACRO.DISPLAY_VERSION ?? MACRO.VERSION}${RESET}`)
  out.push('')
