...
export function buildProfileSaveMessage(
  ...
  if (options?.activatedInSession) {
    lines.push('aethermind switched to it for this session.')
    ...
    lines.push('Restart aethermind to use it.')
  }
...
function ProviderChooser({
  ...
  const helperText = canUseCodexOAuth
    ? 'Save a provider profile without editing environment variables first. Codex profiles backed by env, auth.json, or aethermind secure storage can switch this session immediately when validation succeeds.'
  ...
  if (summary.savedProfileLabel !== 'none') {
    options.push({
      ...
      value: 'clear',
      description: 'Remove .aethermind-profile.json and return to normal startup',
    })
  }
...
function CodexOAuthStep({
  ...
  const handleAuthenticated = React.useCallback(async (
    ...
    if (!env) {
      throw new Error(
        'Codex OAuth succeeded, but aethermind could not build a Codex profile from the stored credentials.',
      )
    }
  ...
  return (
    <Dialog title="Codex OAuth" onCancel={onBack}>
      <Box flexDirection="column" gap={1}>
        <Text>
          Finish signing in with ChatGPT in your browser. aethermind will store
        ...
        {status.browserOpened === false ? (
          ...
          <Text dimColor>
            Browser opened. Complete the sign-in there, then aethermind will
        ...
        <ProviderChooser
          onChoose={value => {
            if (value === 'auto') {
              ...
              onDone(`Removed saved provider profile at ${filePath}. Restart aethermind to go back to normal startup.`, {
                display: 'system',
              })
            } else {
