...
test('CLI identity prefixes describe aethermind instead of Claude Code', () => {
  expect(getCLISyspromptPrefix()).toContain('aethermind')
  ...
  for (const prefix of CLI_SYSPROMPT_PREFIXES) {
    expect(prefix).toContain('aethermind')
    expect(prefix).not.toContain('Claude Code')
    expect(prefix).not.toContain("Anthropic's official CLI for Claude")
  }
})

test('simple mode identity describes aethermind instead of Claude Code', async () => {
  process.env.CLAUDE_CODE_SIMPLE = '1'

  const prompt = await getSystemPrompt([], 'gpt-4o')

  expect(prompt[0]).toContain('aethermind')
  expect(prompt[0]).not.toContain('Claude Code')
  expect(prompt[0]).not.toContain("Anthropic's official CLI for Claude")
})

test('built-in agent prompts describe aethermind instead of Claude Code', () => {
  expect(DEFAULT_AGENT_PROMPT).toContain('aethermind')
  ...
  expect(generalPrompt).toContain('aethermind')
  ...
  expect(explorePrompt).toContain('aethermind')
  ...
  expect(guidePrompt).toContain('aethermind')
  expect(guidePrompt).toContain('You are the aethermind guide agent.')
  expect(guidePrompt).toContain('**aethermind** (the CLI tool)')
