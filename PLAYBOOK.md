# AetherMind Local Agent Playbook

This playbook is a practical guide to run AetherMind with a local model (Ollama), work safely, and get strong day-to-day results.
...
If everything is healthy, AetherMind starts directly.
...
Or let AetherMind recommend the best local model for your goal:
...
```powershell
Get-Content .\.aethermind-profile.json
```
...
```powershell
cd C:\Users\Lucas Pedry\Documents\aethermind\aethermind
bun run dev:profile
```
...
- Keep `.aethermind-profile.json` local (already gitignored).
